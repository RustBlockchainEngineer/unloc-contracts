...
describe('lender claims collateral', async () => {
    // fetch test keypairs
    ...
    const lenderKeypair = await Keypair.generate()
    const borrowerKeypair = await Keypair.generate()
    ...
    const expireLoanDuration = new anchor.BN(1)

    it('lender claims NFT collateral', async () => {
        ...
        if(nftMint) {
            ...
            try{
                // create lender nft token account
                const lenderOfferVault = await nftMint.createAccount(lenderKeypair.publicKey)

                // create lender nft account
                const lenderNftVault = await nftMint.createAccount(lenderKeypair.publicKey)
    
                // claim collateral
                await program.methods.claimCollateral()
                .accounts({
                    lender: lenderKeypair.publicKey,
                    globalState: globalState,
                    treasuryWallet: treasuryKeypair.publicKey,
                ...
                console.log("Error claiming collateral: ", e)
                assert.fail()
            }
        } else {
