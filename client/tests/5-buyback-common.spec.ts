...
describe('buyback-common', () => {
  ...
  it('Is initialized!', async () => {
    ...
    const proxyPreInitializeTx = await ammProxyProgram.rpc.proxyPreInitialize(nonce.toNumber(), {
      accounts: {
    ...
    const proxyInitializeTx = await ammProxyProgram.rpc.proxyInitialize(nonce, new anchor.BN(0), {
        accounts: {
            ammProgram: Liquidity.getProgramId(4),
            amm: ammId,
            ammAuthority: ammAuthority,
    ...
    try {
        const depositTx = await ammProxyProgram.rpc.proxyDeposit(
