...
pub struct ClaimBorrowerRewards<'info> {
    ...
    #[account(mut,
        constraint = borrower_reward_vault.mint == Pubkey::from_str(UNLOC_MINT).unwrap(),
        constraint = borrower_reward_vault.owner == sub_offer.borrower
    )]
