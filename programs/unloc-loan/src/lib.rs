...
pub mod unloc_loan {
    ...
    // admin
    pub fn set_global_state(ctx: Context<SetGlobalState>, accrued_interest_numerator: u64, denominator: u64, min_repaid_numerator: u64, apr_numerator: u64, expire_loan_duration: u64, reward_rate: u64, lender_rewards_percentage: u64, new_super_owner: Pubkey, treasury_wallet: Pubkey) -> Result<()> { 
        process_set_global_state::handle(ctx, accrued_interest_numerator, denominator, min_repaid_numerator, apr_numerator, expire_loan_duration, reward_rate,  lender_rewards_percentage, new_super_owner, treasury_wallet)
    }
