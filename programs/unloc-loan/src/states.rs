...
impl GlobalState {
    pub fn distribute<'info>(
        ...
        require(chainlink_program.key() == chainlink::id(), "wrong chainlink_program")?;
        require(sol_feed.key() == sol_feed_key, "wrong sol_feed")?;
        require(usdc_feed.key() == usdc_feed_key, "wrong usdc_feed")?;
